...
// generate a random ID to attach to a message, edited to 10 characters
export const generateMessageID = () => {
    let randomString = randomBytes(5).toString('hex').toUpperCase();
    const randomPosition = Math.floor(Math.random() * randomString.length);
    const randomLetter = String.fromCharCode(65 + Math.floor(Math.random() * 26));
    randomString = randomString.slice(0, randomPosition) + randomLetter + randomString.slice(randomPosition + 1);
    return 'Tz-' + randomString.slice(0, 10);
};
